...
def generate_plan(weight, height, age, gender, workout_days, goal):
    ...
    workout_plans_df = pd.read_csv("workout_plans.csv")
    
    # Filter the workout plan based on goal and days
    workout_plan = workout_plans_df[
        (workout_plans_df["Goal"] == goal) & (workout_plans_df["Days"] == workout_days)
    ]["Plan"].values
    
    # Handle cases where no plan is found
    workout_plan_text = workout_plan[0] if len(workout_plan) > 0 else "No plan available for the selected days. Try fewer days or a different goal."
